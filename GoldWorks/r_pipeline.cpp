...
namespace gwe {
	...
	gwPipeline::~gwPipeline() {
		vkDestroyShaderModule(workDevice.device(), vertShaderModule, nullptr);
		vkDestroyShaderModule(workDevice.device(), fragShaderModule, nullptr);
	...
	void gwPipeline::createGraphicsPipeline(
		...
		// Vertex Shader Module Configuration
		...
		if (vkCreateGraphicsPipelines(workDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &gwGraphicsPipeline) != VK_SUCCESS) {
			throw std::runtime_error("Graphics Pipeline Creation Error");
		}

	}
	void gwPipeline::createShaderModule(const std::vector<char>& code, VkShaderModule* shaderModule) {
	...
	void gwPipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo) {
